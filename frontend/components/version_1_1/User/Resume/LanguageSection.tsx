  ...
  const LanguageSection = ({
    ...
    <div className=" flex flex-col gap-8">
      <div className="flex items-center gap-3 ">
        ...
        <h3 className="text-3xl font-bold bg-gradient-to-r to-violet-500 from-white bg-clip-text text-transparent">زبان های برنامه نویسی که آشنایی دارم و توضیحات تکمیلی</h3>
      </div>
        ...
        <Card className="border-2 border-violet-900">
            ...
            <CardHeader className="bg-c_background/50 flex justify-between">
              <Chip
                color="secondary"
                variant="light"
                className="bg-black/40 text-tiny p-4"
              >
                ...
                <div className="relative w-full h-full min-h-[220px]">
                  {languages.map((item, index) => {
                    ...
                    return (
                      ...
                      >
                        <Chip
                          className="border-b-2 shadow-lg cursor-pointer"
                          color="secondary"
                          variant="solid"
                        >
  ...
  const EditLanguageModal = ({
    ...
    useEffect(() => {
      ...
      // Only fetch if no initial languages were provided
      fetchLanguages()
    }, [isOpen])
    ...
    // Reset selected languages when modal opens to match initial values
    // useEffect(() => {
    //   if (isOpen) {
    ...
    const toggleLanguage = (language: {
      ...
      if (isLanguageSelected(language.id)) {
        ...
        setSelectedLanguages([...selectedLanguages, { language_name: language.name }]);
      }
    };

    const removeLanguage = (id: string) => {
      setSelectedLanguages(
        selectedLanguages.filter((lang) => lang.id !== id)
      );
    };
                      ...
                      <div className="flex flex-col gap-2">
                        {languages.length > 0 ? (
                          ...
                          <div className="text-center p-4 text-gray-400">
                            زبان برنامه نویسی مورد نظر یافت نشد
                          </div>
                        )}
                      </div>
                    </ScrollShadow>
