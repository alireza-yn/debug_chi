import { Debug } from "@/components/types/activity";
...
const HistoryCard = (props: Props) => {
  const people = [
    {
      id: 1,
      name: `${props.data.debuger.first_name} ${props.data.debuger.last_name}`,
      designation: "Front-End Developer",
      image: `${process.env.server}/${props.data.debuger.image_profile}`,
    },
  ];
  return (
      ...
      <div className="flex flex-col box-border px-4 gap-4 ">
        {
          ...
          <Chip startContent={<MessageCircleCode size={14}/>} variant="flat" color="secondary">چت</Chip>
        }
        <div className="flex flex-col rounded-lg bg-c_background gap-2 text-xs p-4" dir="ltr">
        <div className=" flex items-center justify-between">
          <div className="flex flex-col items-center justify-end gap-2">

          <h3>
            {props.data.status == "close" && 
          <Chip  variant="flat" color="danger">پایان یافته</Chip>
            }
            {props.data.status == "open" && 
            <span>

                  {new Date(props.data.created_at).toLocaleDateString('fa-IR',{
                    weekday:"long",
                    day:"numeric",
                    month:"long",
                    year:"numeric"
        ...
        <h3 className="text-lime-300">{props.data.title || "نوع درخواست"}</h3>
          ...
          <Button color="primary" variant="flat">دیدن اطلاعات</Button>
      </div>
    </div>
  );
};
